...
bool DekoCompiler::CompileGlsl(const char* glsl)
{
	...
	if (m_stage == pipeline_stage_vertex)
		dump_shader_info(m_glsl, "symbols.vert.json");
	if (m_stage == pipeline_stage_fragment)
		dump_shader_info(m_glsl, "symbols.frag.json");
	if (m_stage == pipeline_stage_geometry)
		dump_shader_info(m_glsl, "symbols.geom.json");
